...
const SYSTEM_PROMPT = `أنت "وكيل الذاكرة" في فريق دعم تقني للموظفين السعوديين.

مهمتك:
1. ابحث في سجل البلاغات السابقة باستخدام أداة searchPastTickets.
2. حلّل النتائج وأوصِ بأحد المسارات الثلاثة.
...
قواعد:
- استخدم searchPastTickets مرة واحدة على الأقل.
- فضّل "scripted" دائماً للمهام المعروفة — أسرع وأكثر موثوقية.
- لا تستخدم أي أداة غير searchPastTickets.`;

export async function runMemoryAgent(transcript: string, anthropicKey: string): Promise<MemoryResult> {
